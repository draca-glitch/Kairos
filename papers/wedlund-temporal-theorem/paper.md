...
Layer 5 ships in Kairos v0.4.0 as `mcp/temporal-future.py`, an MCP server with two tools: `temporal_future_query(horizon_days)` returns overdue / due-today / upcoming items across a forward window, plus expiring memories (memories carrying a `valid_until` field whose date falls inside the horizon), plus a `highlights` list of one-line summaries; `temporal_obligations_for(area, horizon_days)` narrows the same query to a single domain. Routing rule R8 ("prompt mentions deadline / upcoming / schedule / tomorrow / planera / ...") surfaces the tool automatically without the model needing to remember to call it, so forward-state enters the response pipeline whenever the user's prompt anchors on it.
