...
2. **`hooks/temporal-state.py`**: sibling of `time.sh`. Where time.sh gives the raw timestamp, this one computes the *shape* of time and prepends it as a single-line summary: the user's gap, cross-day status, time-of-day bucket, input-cadence (rapid-fire vs reflective vs resumed-after-gap), and session-phase (continuing vs interruption-pivot vs session-start). Lets Claude arrive at the prompt with computed temporal context already grounded instead of having to derive it each turn. Measurement and inference are kept apart: `gap=40s(since-reply) | turn=15m` are two facts (the user's pause since the assistant finished, and the raw prompt-to-prompt time), `cadence=active-collaboration` is the reading drawn from the first one. When the assistant's reply timestamp is not available the gap is measured prompt-to-prompt, labelled `since-prompt`, and cadence carries `(turn-basis)`: a long pause on that basis may be the assistant's own tool time, not the user thinking. Claude Code transcripts stamp every assistant record, so reply timing is free there; harnesses on the thread-ring backend get it by running `hooks/turn-end.py` on their turn-end event.
3. **`hooks/temporal-routing.py`**: Layer 6. Translates the temporal state into deterministic tool-routing advisories. Advisories describe the user's rhythm and never override what the prompt asks for: `skip=preamble` on rapid-fire means trim ceremony, not shorten the answer, and a cadence rule that fired names its basis (`basis=reply` or `basis=turn`) so the reader can weigh it. R5 (reflective, write more) only fires on reply basis. (`suggest=memory_search-first`, `skip=TaskCreate-overhead`, etc.) so the temporal signal isn't only informational but procedural. Eight rules v0: long-gap → memory-first, cross-day → staleness flag, rapid-fire → skip overhead, session-start → read CLAUDE.md, reflective + long prompt → write-longer-reasoning-prose, late-night-resumed → confirm-before-destructive, tech-keyword prompt → temporal_staleness_audit-first (R7, integrates Layer 3), forward-time prompt → temporal_future_query-first (R8, integrates Layer 5). Silent when no rule fires.
